...
func (h *Handler) healthScheduler() []healthItem {
	...
	items = append(items, h.fixedTaskItem("kube", "容器集群检查", h.Cfg.KubeCheckSpec,
	...
	items = append(items, h.fixedTaskItem("hostlog", "主机日志巡检", h.Cfg.HostLogSpec,
		"OPS_HOST_LOG_SPEC 为空，日志里新冒出来的错误与「日志目录快把盘写满了」都只能手动巡检"))
