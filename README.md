...
- **集群资源改动与下钻**：看单个对象的 YAML（自动去掉 status 与 resourceVersion / uid / managedFields 这类集群自己维护的字段，改完可直接提交），走 Kubernetes **服务端 apply** 落盘，提交前可「预检」让 API Server 完整校验（含准入控制）但不落盘；Deployment / StatefulSet 还能直接改副本数，Deployment / StatefulSet / DaemonSet 支持**滚动重启**（与 `kubectl rollout restart` 同一做法：给 Pod 模板打时间戳注解让控制器换掉 Pod，界面上明说这不是原地重启容器）。字段被 kubectl 或别的控制器管着时会明确报冲突，确认后可勾「强制接管」。资源浏览覆盖 **17 种类型**，按工作负载 / 网络 / 配置 / 存储 / 集群分组，支持名称过滤与分页；对象详情除了 YAML 还有**它自己的事件**（按 `involvedObject` 反查，不用在整命名空间的事件里翻）和**关联 Pod**（按 spec.selector 的等值标签查，与集群自己的选法一致；用 matchExpressions 选不出来时如实说明，不按名字前缀猜）。三档权限界线摆在界面上：可写（Deployment / StatefulSet / DaemonSet / CronJob / Job / Service / Ingress / ConfigMap / PVC）、**只读**（Pod / ReplicaSet / Endpoints / Namespace / Node / PV / StorageClass —— 由控制器维护，改了也会被改回去，因此不给编辑入口）、**脱敏只读**（Secret 只给键名不给值，那份 YAML 也禁止提交，否则会把真实值覆盖成占位符）。**不提供删除、不开放 RBAC**——要动这些仍然走 kubectl。写操作单独要 `kube:write` 权限，每次改动（apply / scale / restart，含预检与失败）都留痕，记下对象、提交内容、API Server 的原话与操作人，保留天数纳入「数据留存」
