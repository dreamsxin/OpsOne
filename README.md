...
- 配了 `OPS_SECRET_KEY` 时，**18 列凭据**都以 **AES-GCM 密文**落库：凭证库、主机自带凭据、容器集群 kubeconfig、数据库实例凭据，以及 LDAP 服务账号口令、IM AppSecret、云账号 AK/SK、Jenkins Token、模型上游 Key、通知渠道签名密钥与鉴权头、指标 / 日志 / 链路三个数据源的请求头、SMTP 口令、账号密码库口令与 2FA 托管种子。仍是明文的只剩 TOTP 密钥与告警推送令牌两项（原因写在 SECURITY 第 2 节），「凭证库 → 密钥加密体检」会逐列列出密文 / 明文行数。同时拿到数据库与 `OPS_SECRET_KEY` 的人依然能拿到全部凭据 —— 加密挡的是只拿到库文件或备份的那类场景。注意最后两列的加密**只防库文件被拷走**，防不了「有权限的人取明文」（那是设计如此），后者靠取用留痕兜
