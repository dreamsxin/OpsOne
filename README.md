...
- `OPS_SECRET_KEY` — 凭据字段的加密密钥（任意长度口令，内部派生 256 位），覆盖凭证库、主机凭据、kubeconfig、数据库口令与各类对外系统密钥（含 SMTP 口令）。**留空就是不加密存储**，这是被允许的配置，页面上会照实标注；要换密钥或退回明文，走「凭证库 → 密钥加密体检 → 更换密钥 / 取消加密」（会先整库备份再逐行重写，并要求你把这个环境变量同步改掉），不要直接改这个变量
...
- **不加密存储是允许的**：`OPS_SECRET_KEY` 留空即明文落库，界面会把当前状态写成「不加密存储」并说清代价。加密开关也不是单向的 —— 体检卡片上的「更换密钥 / 取消加密」能换成另一把密钥、也能解回明文：先整库备份，再逐行解开重写（解不开的行原样留着并报出来，不会被清空），进程内的密钥立刻切换、不用停机，但你要按提示把 `OPS_SECRET_KEY` 同步改掉
